 ...
 * processa: `kie:<taskId>` (Kling V3 Turbo via kie.ai) ou `run:<runId>`
 * (ComfyDeploy, legado/fallback). Este módulo centraliza a resolução do estado
 * pra que /api/video-status, o reconcile e o cron compartilhem a mesma lógica.
 */

export function findVideoMarker(inputUrls?: string[] | null): string | null {
  return (inputUrls ?? []).find((u) => u.startsWith('kie:') || u.startsWith('run:')) ?? null;
}
